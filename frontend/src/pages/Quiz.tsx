...
export default function QuizPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t('quiz.error_generate')));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t('quiz.error_submit')));
    } finally {
      setLoading(false);
    }
  };

  const handleLoadWrong = useCallback(async () => {
    try {
    ...
    try {
      ...
      if (res.removed) {
        ...
        setWrongAnswers((prev) =>
          prev.map((w) => (w.id === id ? { ...w, mastery_level: res.mastery_level } : w))
        );
      }
    } catch {
      /* ignore */
  ...
  const handleExportAnki = async () => {
    try {
      ...
      setError(getErrorMessage(error, t('quiz.error_export')));
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="spell-card bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {docs.length === 0 && (
                  <p className="text-xs text-gray-400">{t('quiz.noReadyDocs')}</p>
                )}
              </div>
              <div className="flex items-center gap-3">
                <label className="text-sm text-gray-600">{t('quiz.questionCount')}</label>
                <select
                  value={questionCount}
            ...
            {quiz && !result && (
              <div className="space-y-4">
                ...
                >
                  {loading
                    ? t('common.submitting')
                    : `${t('quiz.submitAnswer')} (${answeredCount}/${quiz.total_count})`}
                </button>
              </div>
            )}
              ...
              wrongAnswers.map((w) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    >
                      {t('quiz.masteryLevel', { level: w.mastery_level })}
                    </span>
                  ...
                  <div className="flex gap-3 text-xs mb-3">
                    <span className="px-2.5 py-1 rounded-lg bg-red-50 text-red-600 border border-red-100">
                      {t('quiz.yourAnswer')}
                      {w.user_answer}
                    </span>
                  ...
                  {w.explanation && (
                    <p className="text-xs text-gray-500 mb-3 bg-gray-50 rounded-lg p-2.5 leading-relaxed">
                      {w.explanation}
                    </p>
                  )}
